...
import {all, createRef, easeInOutCubic, easeOutBack, waitUntil} from '@motion-canvas/core';
...
function Person({name, color}: {name: string; color: string}) {
  return (
    <Layout>
      ...
      <Line points={[[0, -60], [0, 90]]} stroke={color} lineWidth={32} lineCap={'round'} />
      <Line points={[[-68, -5], [0, -42], [68, -5]]} stroke={color} lineWidth={25} lineCap={'round'} />
      <Line points={[[-55, 175], [0, 85], [55, 175]]} stroke={color} lineWidth={27} lineCap={'round'} />
...
export default makeScene2D(function* (view) {
  ...
  view.add(
    <>
      ...
      <Txt ref={borrowTxt} text={"借用"} x={120} y={-280} fill={BLUE} fontSize={30} opacity={0} />

      <Line
        ref={ownerLink}
        points={[[200, -230], [-40, -230]]}
        stroke={BLUE}
        lineWidth={8}
        endArrow
        arrowSize={22}
      ...
      <Layout ref={key} x={-140} y={-230} scale={0.82}>
        ...
        <Line points={[[-12, 0], [90, 0]]} stroke={YELLOW} lineWidth={22} lineCap={'round'} />
        <Line points={[[25, 0], [25, 32]]} stroke={YELLOW} lineWidth={16} />
        <Line points={[[61, 0], [61, 25]]} stroke={YELLOW} lineWidth={16} />
      </Layout>

      <Layout ref={choices} layout direction={'column'} y={380} gap={40} opacity={0}>
        <Rect ref={moveChoice} width={880} height={150} radius={28} fill={PANEL} stroke={'#34405C'} lineWidth={3}>
          <Layout layout alignItems={'center'} padding={[0, 42]} gap={28}>
            <Txt text={'直接交出去'} fill={TEXT} fontWeight={700} fontSize={39} />
            <Txt text={'Move'} fill={YELLOW} fontFamily={'JetBrains Mono, monospace'} fontWeight={800} fontSize={43} />
          </Layout>
        </Rect>
